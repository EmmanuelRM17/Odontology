...
const BarraEmpleados = ({ onDrawerChange }) => {
  ...
  // Estructura de menú organizada por grupos para empleados
  const menuGroups = [
    {
      id: 'gestion',
      title: 'Gestión',
      items: [
        { icon: MdPeople, text: 'Gestión de Pacientes', path: '/Empleado/pacientes' },
        { icon: MdMedicalServices, text: 'Gestión de Servicios', path: '/Empleado/servicios' },
        { icon: MdEvent, text: 'Gestión de Citas', path: '/Empleado/citas' },
        { icon: MdLocalHospital, text: 'Gestión de Tratamientos', path: '/Empleado/tratamientos' },
        { icon: FaFileMedical, text: 'Expediente Clínico', path: '/Empleado/expedienteClinico' },
        { icon: MdSchedule, text: 'Gestión de Horarios', path: '/Empleado/horarios' },
        { icon: MdCloudUpload, text: 'Subida de Imágenes', path: '/Empleado/imagenes' }
      ]
    },
    {
      id: 'reportes',
      ...
      items: [
        { icon: MdShowChart, text: 'Estadísticas', path: '/Empleado/estadisticas' },
        { icon: MdDescription, text: 'Reportes', path: '/Empleado/reportes' },
        { icon: MdRateReview, text: 'Reseñas', path: '/Empleado/reseñas' }
      ]
    },
    {
      id: 'sistema',
      title: 'Sistema',
      items: [
        { icon: MdSettings, text: 'Configuración', path: '/Empleado/configuracion' }
      ]
    }
  ];
  ...
  useEffect(() => {
    const checkAuthStatus = async () => {
      try {
        ...
        // Usar allAuthenticatedUsers para obtener el empleado
        if (data.authenticated && data.allAuthenticatedUsers && data.allAuthenticatedUsers.empleado) {
          setUser(data.allAuthenticatedUsers.empleado);
          console.log('✅ Empleado autenticado correctamente');
        } else {
          console.log('❌ No hay sesión de empleado activa');
          setUser(null);
        }
      } catch (error) {
  ...
  const colors = {
    ...
    primary: isDarkTheme ? '#3B82F6' : '#2563EB', // Azul para empleados
    secondary: isDarkTheme ? '#10B981' : '#047857',
    ...
    <>
      ...
      >
        ...
        <Typography variant="body2" color={colors.secondaryText}>
          {user?.email || 'empleado@odontologiacarol.com'}
        </Typography>
      </Box>
      ...
      >
        ...
        >
          ...
          <Paper
            ...
            sx={{
              backgroundColor:
                location.pathname === '/Empleado/principal'
                  ? colors.activeItem
                  : 'transparent',
              borderRadius: 1.5,
              overflow: 'hidden',
          ...
          >
            <ListItem
              button
              onClick={() => handleItemClick({ path: '/Empleado/principal' })}
              disableRipple
              sx={{
                py: 1.5,
                px: 1.5
            ...
            >
              ...
              <ListItemText
                ...
                primaryTypographyProps={{
                  fontSize: '0.9rem',
                  fontWeight:
                    location.pathname === '/Empleado/principal' ? 600 : 500,
                  color:
                    location.pathname === '/Empleado/principal'
                      ? colors.primary
                      : colors.text
          ...
          <Paper
            ...
            sx={{
              backgroundColor:
                location.pathname === '/Empleado/ayuda'
                  ? colors.activeItem
                  : 'transparent',
              borderRadius: 1.5,
              overflow: 'hidden',
          ...
          >
            <ListItem
              button
              onClick={() => handleItemClick({ path: '/Empleado/ayuda' })}
              disableRipple
              sx={{
                py: 1.2,
                px: 1.5
            ...
            >
              ...
              <ListItemText
                ...
                primaryTypographyProps={{
                  fontSize: '0.875rem',
                  fontWeight: location.pathname === '/Empleado/ayuda' ? 600 : 400,
                  color:
                    location.pathname === '/Empleado/ayuda'
                      ? colors.primary
                      : colors.text
      ...
      >
        <Toolbar sx={{ display: 'flex', justifyContent: 'space-between' }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
            ...
            <IconButton
              component={Link}
              to="/Empleado/calendarioCita"
              sx={{
                color:
                  location.pathname === '/Empleado/calendarioCita'
                    ? colors.primary
                    : colors.iconColor,
                '&:hover': { color: colors.primary, backgroundColor: colors.hover }
              }}
