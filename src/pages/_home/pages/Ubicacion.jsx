...
const UbicacionHorarios = () => {
  ...
  const toggleMapView = () => setMapView(mapView === 'roadmap' ? 'satellite' : 'roadmap');
  ...
  const pageAnimationVariants = {
    hidden: { opacity: 0 },
    visible: { opacity: 1, transition: { duration: 0.6, when: "beforeChildren", staggerChildren: 0.15 } },
    exit: { opacity: 0, transition: { duration: 0.3 } }
  };

  const titleAnimationVariants = {
    hidden: { opacity: 0, y: -20 },
    visible: { opacity: 1, y: 0, transition: { type: "spring", stiffness: 80, damping: 10, duration: 0.5 } }
  };

  const contentAnimationVariants = {
    hidden: { opacity: 0, y: 30, scale: 0.98 },
    visible: { opacity: 1, y: 0, scale: 1, transition: { type: "spring", stiffness: 50, damping: 10, duration: 0.6 } }
  };

  const cardVariants = {
    hidden: { opacity: 0, scale: 0.95 },
    visible: { opacity: 1, scale: 1, transition: { type: "spring", stiffness: 50, damping: 12, duration: 0.6 } },
    hover: { scale: 1.01, boxShadow: "0px 10px 25px rgba(0,0,0,0.15)", transition: { duration: 0.2 } }
  };
  ...
  const floatingIconVariants = {
    initial: { y: 0 },
    animate: { y: [-3, 3, -3], transition: { duration: 2.5, repeat: Infinity, ease: "easeInOut" } }
  };

  const tabItemVariants = {
    inactive: { color: colors.secondaryText, backgroundColor: 'transparent' },
    active: { color: colors.primaryColor, backgroundColor: colors.tabActive },
      ...
      >
        ...
        >
          ...
          <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.4 }}>
            <Typography variant="h5" sx={{ color: colors.primaryText, fontWeight: 600, mb: 2 }}>Cargando mapa</Typography>
            <Typography variant="body1" sx={{ color: colors.secondaryText, mt: 1, textAlign: 'center', maxWidth: '80%', mx: 'auto' }}>
              Estamos preparando la ubicación de Clínica Dental Carol para que puedas encontrarnos fácilmente.
            </Typography>
            <motion.div animate={{ opacity: [0.5, 1, 0.5] }} transition={{ duration: 1.8, repeat: Infinity, ease: "easeInOut" }} style={{ marginTop: '20px' }}>
              <Typography variant="body2" color={colors.primaryColor}>Por favor, espera un momento...</Typography>
            </motion.div>
          </motion.div>
        </motion.div>
      </Box>
  ...
  if (loadError || apiLoadError) {
    return (
      <Box sx={{ p: 4, textAlign: 'center', background: colors.background, borderRadius: '12px', boxShadow: colors.cardShadow }}>
        <motion.div initial={{ opacity: 0, y: -15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
          <Typography color="error" variant="h5" gutterBottom sx={{ fontWeight: 600, mb: 2.5 }}>Error al cargar el mapa</Typography>
          <Typography color={colors.secondaryText} variant="body1" sx={{ mb: 3, maxWidth: '600px', mx: 'auto' }}>
        ...
        >
          ...
          <Container maxWidth="lg" sx={{ py: 4, position: 'relative', zIndex: 1 }}>
            <motion.div variants={titleAnimationVariants}>
              ...
              <Typography variant="subtitle1" align="center" sx={{ color: colors.secondaryText, maxWidth: '600px', mx: 'auto', mb: 4, fontSize: '1rem' }}>
            ...
            {isMobile && (
              <motion.div variants={staggerItemVariants}>
                <Box sx={{ display: 'flex', justifyContent: 'center', mb: 3, borderRadius: '10px', backgroundColor: isDarkTheme ? alpha('#ffffff', 0.04) : alpha('#000000', 0.02), p: 0.6, boxShadow: '0 2px 8px rgba(0,0,0,0.03)' }}>
                  <motion.div animate={activeTab === 'ubicacion' ? 'active' : 'inactive'} whileHover="hover" variants={tabItemVariants} style={{ borderRadius: '8px', padding: '10px 16px', cursor: 'pointer', flex: 1, textAlign: 'center' }} onClick={() => setActiveTab('ubicacion')}>
                    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 1 }}>
                      <LocationOn fontSize="small" />
                      <Typography variant="button" fontWeight={600}>Ubicación</Typography>
                    </Box>
                  </motion.div>
                  <motion.div animate={activeTab === 'horarios' ? 'active' : 'inactive'} whileHover="hover" variants={tabItemVariants} style={{ borderRadius: '8px', padding: '10px 16px', cursor: 'pointer', flex: 1, textAlign: 'center' }} onClick={() => setActiveTab('horarios')}>
                    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 1 }}>
                      <CalendarMonth fontSize="small" />
                      <Typography variant="button" fontWeight={600}>Horarios</Typography>
                    </Box>
                  </motion.div>
                </Box>
              </motion.div>
              ...
              <Grid container spacing={3}>
                <Grid item xs={12} md={8}>
                  <motion.div variants={contentAnimationVariants}>
                    <Card elevation={0} sx={{ backgroundColor: colors.cardBackground, borderRadius: '12px', overflow: 'hidden', boxShadow: colors.cardShadow, border: `1px solid ${alpha(colors.divider, 0.5)}`, height: '100%', position: 'relative', '&::before': { content: '""', position: 'absolute', top: 0, left: 0, width: '100%', height: '3px', background: colors.accentGradient } }}>
                      <CardContent sx={{ p: { xs: 2, sm: 3 } }}>
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                          <Typography variant="h6" sx={{ fontWeight: 600, color: colors.primaryText, display: 'flex', alignItems: 'center', gap: 1 }}>
                              ...
                              <InfoWindow position={center} onCloseClick={() => setShowInfoWindow(false)} options={{ pixelOffset: new window.google.maps.Size(0, -35), maxWidth: 300 }}>
                                <Box sx={{ p: 1, maxWidth: 280 }}>
                                  ...
                                  <Typography variant="body2" sx={{ mb: 1, display: 'flex', alignItems: 'flex-start', gap: 1 }}>
                                    <LocationOn sx={{ fontSize: 18, color: "#059669", mt: 0.3 }} /><span>{clinicInfo.direccion}</span>
                                  </Typography>
                                  <Typography variant="body2" sx={{ mb: 1, display: 'flex', alignItems: 'center', gap: 1 }}>
                                    <AccessTime sx={{ fontSize: 16, color: "#059669" }} /><span style={{ color: "#059669", fontWeight: 500 }}>Abierto ahora</span>
                                  </Typography>
                                  <Typography variant="body2" sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                                    <Star sx={{ fontSize: 16, color: "#FF9800" }} /><span>{clinicInfo.reseñas}</span>
                                  </Typography>
                                </Box>
                              </InfoWindow>
                            )}
                ...
                <Grid item xs={12} md={4}>
                  <Box sx={{ mb: 3 }}>
                    <motion.div variants={cardVariants} whileHover="hover">
                      <Card elevation={0} sx={{ backgroundColor: colors.cardBackground, borderRadius: '12px', overflow: 'hidden', boxShadow: colors.cardShadow, border: `1px solid ${alpha(colors.divider, 0.5)}`, position: 'relative', '&::before': { content: '""', position: 'absolute', top: 0, left: 0, width: '100%', height: '3px', background: colors.accentGradient } }}>
                        <CardContent sx={{ p: { xs: 2, sm: 3 } }}>
                          <motion.div variants={staggerItemVariants}>
                            <Typography variant="h6" sx={{ fontWeight: 600, color: colors.primaryText, mb: 2.5, display: 'flex', alignItems: 'center', gap: 1 }}>
                              <Info sx={{ color: colors.primaryColor, fontSize: '1.2rem' }} /> Información de Contacto
                            </Typography>
                          </motion.div>
                          <motion.div variants={staggerItemVariants}>
                            <Box sx={{ mb: 2.5 }}>
                              ...
                              <Box sx={{ p: 1.8, borderRadius: '10px', bgcolor: isDarkTheme ? alpha('#ffffff', 0.04) : alpha('#2563EB', 0.02), mb: 2, transition: 'all 0.2s ease', '&:hover': { transform: 'translateX(3px)', bgcolor: isDarkTheme ? alpha('#ffffff', 0.06) : alpha('#2563EB', 0.04) } }}>
                                ...
                                <Typography variant="body2" sx={{ color: colors.secondaryText, pl: 3.5, fontSize: '0.8rem', lineHeight: 1.5 }}>{clinicInfo.direccion}</Typography>
                              </Box>
                              <Box sx={{ p: 1.8, borderRadius: '10px', bgcolor: isDarkTheme ? alpha('#ffffff', 0.04) : alpha('#2563EB', 0.02), mb: 2, transition: 'all 0.2s ease', '&:hover': { transform: 'translateX(3px)', bgcolor: isDarkTheme ? alpha('#ffffff', 0.06) : alpha('#2563EB', 0.04) } }}>
                                <Typography variant="subtitle2" sx={{ fontWeight: 600, color: colors.primaryText, mb: 0.5, display: 'flex', alignItems: 'center', gap: 1, fontSize: '0.85rem' }}>
              ...
              <Grid item xs={12}>
                {(!isMobile || activeTab === 'horarios') && (
                  <motion.div variants={contentAnimationVariants}>
                    <Card elevation={0} sx={{ backgroundColor: colors.cardBackground, borderRadius: '12px', overflow: 'hidden', boxShadow: colors.cardShadow, border: `1px solid ${alpha(colors.divider, 0.5)}`, position: 'relative', '&::before': { content: '""', position: 'absolute', top: 0, left: 0, width: '100%', height: '3px', background: colors.accentGradient } }}>
                      <CardContent sx={{ p: { xs: 2, sm: 3 } }}>
                        <HorariosAtencion colors={colors} titleAnimationVariants={titleAnimationVariants} staggerItemVariants={staggerItemVariants} />
                      </CardContent>
                    </Card>
                  </motion.div>
                )}
...
export default memo(UbicacionHorarios);